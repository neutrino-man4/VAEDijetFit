...
if __name__ == "__main__":
   ...
   seed=-1 
   mass = options.mass; mass_label=(int)(mass)
   ...
   for iq,q in enumerate(quantiles):
      ...
      for i, nPars in enumerate(nParsToTry):
         ...
         fres = fitter_QCD.fit('model_b','data_qcd',[ROOT.RooFit.Save(1), ROOT.RooFit.Verbose(0), ROOT.RooFit.Minos(1)],out_dir)
         fres.Print()
         print "**********************************************************************************************"
         ...
         qcd_fnames.append(str(nPars) + 'par_qcd_fit%i_quantile%s.root' % (i,q))         
      ...
      print(chi2s[iq])
      print(best_i[iq])
      best_i[iq] = f_test(nParsToTry_converged, ndofs[iq], chi2s[iq])
      #import pdb; pdb.set_trace()
      nPars_QCD[iq] = nParsToTry_converged[best_i[iq]]
      ...
      # ARITRA: Add signal shape obtained by fitting specific signal which was fed as input. Alternative: Use standard XtoYY shape, see below 
      ...
      ## Add standard X to YY signal shape as agreed with Oz
      ...
      binsx_mutable=copy.deepcopy(binsx)
      
      
      CHI2,NDOF=checkSBFit('{out_dir}/workspace_JJ_{xsec}_{label}.root'.format(out_dir=out_dir, xsec=sig_xsec,label=q),q,roobins,histos_qcd[iq].GetName()+"_M{mass}_xsec{xsec}".format(mass=mass,xsec=sig_xsec), nPars_QCD[iq], out_dir)
      PVAL=ROOT.TMath.Prob(CHI2,NDOF)
      # while (True):
      #    roobins_mutable = ROOT.RooBinning(len(binsx_mutable)-1, array('d',binsx_mutable), "mjjbins")
      #    CHI2,NDOF=checkSBFit('{out_dir}/workspace_JJ_{xsec}_{label}.root'.format(out_dir=out_dir, xsec=sig_xsec,label=q),q,roobins_mutable,histos_qcd[iq].GetName()+"_M{mass}_xsec{xsec}".format(mass=mass,xsec=sig_xsec), nPars_QCD[iq], out_dir)
      #    PVAL=ROOT.TMath.Prob(CHI2,NDOF)
         
         
      #    if PVAL>0.05: break
      #    print "P-value < 0.05. Starting fit from next bin."
      #    print "\n \n \n \n \n \n \n \n"
      #    binsx_mutable=binsx_mutable[1:]
      ...
      if q=='q90':
         ...
         fields=[mass,binsx_mutable[0],PVAL,pvalue_sb]
   ...
   for iq,q in enumerate(quantiles):  
      ...
      # Add standard XtoYY shape for M = 3TeV
      if dcb: card.addSignalShapeDCB('model_signal_mjj','mjj', 'XToYY_CASE_shapes/case_interpolation_M%s.0.root'%(str(mass_label)), {'CMS_scale_j':1.0}, {'CMS_res_j':1.0})
      ...
      if options.correlateB == True:
         ...
         else: card.addFloatingYieldCorr('model_qcd_mjj',1, os.path.join(out_dir, qcd_fname[iq]), "mjj_qcd_%s"%q,fractions[iq], constant=histos_qcd[0].GetEntries())
   ...
   for iq,q in enumerate(quantiles): 
      if q == 'total': continue
      CHI2,NDOF=checkSBFitFinal(out_dir+'/workspace_JJ_{xsec}_{label}.root'.format(xsec=sig_xsec,label='final'),q,roobins,'M{mass}_xsec{xsec}_{q}_final'.format(mass=mass,xsec=sig_xsec,q=q),nPars_QCD[0],out_dir)
      PVAL=ROOT.TMath.Prob(CHI2,NDOF)